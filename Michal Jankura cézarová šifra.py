...
znaky = "+-*/?.!:@&#"
cisla = "0123456789"
def pocet_znakov(nazov_suboru, Abeceda,abeceda,znaky,cisla):
    file = open(nazov_suboru, "r")
    text = file.read()
    pocet_znkv = 0
    for i in text:
        if i == Abeceda or abeceda or znaky or cisla:
            pocet_znkv += 1
    return pocet_znkv
print("Pocet zankov je : ",pocet_znakov('zasifruj.txt',Abeceda,abeceda,znaky,cisla))

def sifruj_subor():
    with open("zasifruj.txt","r",encoding="UTF-8") as file:
        subor = file.read()
        pocet_slov = len(subor)
        print(f"Počet slov je {pocet_slov}")
        text = subor
        print("Pôvodná správa : ",text)
        kluc  = int(input("Zadaj kľúč : "))
        if kluc >=27:
            ...
            with open("zasifruj2.txt","w",encoding="UTF-8") as file2:
                file2.write(sifruj(text,kluc))
